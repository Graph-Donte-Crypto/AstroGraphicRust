...
fn main() {
	...
	let axis_0 = Point3::from([-box_size, -box_size, -box_size]);
	let axis_x = Point3::from([box_size, -box_size, -box_size]);
	let axis_y = Point3::from([-box_size, box_size, -box_size]);
	let axis_z = Point3::from([-box_size, -box_size, box_size]);

	let mut camera =
		kiss3d::camera::FirstPerson::new(Point3::from([0.0, 0.0, 0.0]), Point3::from([-1.0, -1.0, -1.0]));
	...
	while window.render_with_camera(&mut camera) {
		...
		window.draw_line(&axis_0, &axis_x, &Point3::from([1.0, 0.0, 0.0]));
		window.draw_line(&axis_0, &axis_y, &Point3::from([0.0, 1.0, 0.0]));
		window.draw_line(&axis_0, &axis_z, &Point3::from([0.0, 0.0, 1.0]));
