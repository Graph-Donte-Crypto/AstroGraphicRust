...
H_M = H_min / np.array([[dMdE1 * dMdE1, dMdE1 * dMdE2], [dMdE1 * dMdE2, dMdE2 * dMdE2]])
...
ell_local = (
    semi[0] * np.cos(t_ell)[:, None] * eigvecs[:, 0]
...
print(
    f"Ascending  node (+ell): M1={np.degrees(M1_plus):.3f} deg, "
    f"M2={np.degrees(M2_plus):.3f} deg, |r1-r2|={d_plus:.3e} km "
    f"({d_plus / r_soi:.2f} r_SOI)"
)
print(
    f"Descending node (-ell): M1={np.degrees(M1_minus):.3f} deg, "
...
print("=" * 78)
print(
    f"True minimum (converged Newton): "
...
def report(name, E1_0, E2_0):
    ...
    converged = (
        abs(wrap_pi(E1f - E1_star)) < 1e-6 and abs(wrap_pi(E2f - E2_star)) < 1e-6
    )
    print(f"\n[{name}]")
    print(
        f"  guess: E1={E1_0:.4f} rad ({np.degrees(E1_0):.4f} deg), "
    ...
    print(
        f"  Newton iters to converge: {iters}  "
...
cs_f = ax.contour(
    np.degrees(M1g), np.degrees(M2g), F, levels=[0], colors="black", linewidths=1.5
)
ax.text(
    0.02,
    0.98,
...
dt_sc_to_j2000 = (
    datetime(2000, 1, 1, 12, 0, 0) - datetime(1977, 8, 23, 11, 29, 11)
).total_seconds()
...
for k1 in range(-5, 6):
    for k2 in range(-5, 6):
        ...
        if M2_line[0] > np.radians(130) or M2_line[-1] < 0:
            continue
        ax.plot(np.degrees(M1_line), np.degrees(M2_line), color="gray", linewidth=0.7)
...
ax.plot(
    np.degrees(M1_line),
    np.degrees(slope * M1_line + offset_star),
    color="green",
    linewidth=1.0,
    linestyle=":",
...
ax.set_title(
    "Distance constraint $f=0$ in mean anomaly domain\nVoyager 2 — Jupiter", fontsize=14
)
ax.set_xlim(0, 120)
ax.set_ylim(0, 130)
